@echo off 

set render_search_range=0 1
set spp=128

for %%r in (%render_search_range%) do (
    C:\Users\tsvdh\CodeProjects\Clion\AcceleratedVolRenderer\build\Release\pbrt.exe ^
    C:\Users\tsvdh\CodeProjects\VSCode\PbrtScenes\disney-cloud\render_search_range\disney-cloud.pbrt ^
    --display-server localhost:14158 --spp %spp% --integrator graph ^
    --graph-data C:\Users\tsvdh\CodeProjects\VSCode\PbrtScenes\disney-cloud\render_search_range\graphs\disney-cloud_render_search_range%%r.txt ^
    --outfile C:\Users\tsvdh\OneDrive\Documenten\thesis\results\disney-cloud_render_search_range\graph\disney-cloud_graph_render_search_range-%%r.exr
)
